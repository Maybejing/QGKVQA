...
def load_FVQA_data(cfg, args):
    ...
    for i, image_fn in enumerate(image_fns):
        if image_fn in train_fns:
            ...
            trainA.append(ans[i])
            trainKB.append(kb[i])
            trainImg.append(image_fns[i])

        elif image_fn in test_fns:
            testQ.append(ques[i])
            testA.append(ans[i])
            testKB.append(kb[i])
            testImg.append(image_fns[i])
...
class FVQA(Dataset):
    def __init__(self, cfg, args, data,transformer):
        self.cfg = cfg
        self.args = args
    ...
    def __getitem__(self, idx):
        if self.args.model_name == "ht":
            ques = np.array(self.data["ques"][idx], dtype=np.int64)
            ...
            img=Image.open("data/fvqa/images/"+self.data["image"][idx]).convert("RGB")
            return ques, ques_out, kg_out, self.transformer(img), ans

   

class KVQA(Dataset):
    def __init__(self, cfg, args, mode,transformer,task_idx=-1):
        self.cfg = cfg
        self.args = args
        self.transformer=transformer
    ...
    def __getitem__(self, idx):
        if self.args.model_name == "ggnn":
            ...
            q=np.zeros((17))
            if len(ques) > 17:
                q[: 17] = np.array(ques)[:17]
            else:
                q[: len(ques)] = np.array(ques)
            ...
            try:
                img=Image.open("data/kvqa/"+self.data["imgs"][idx]).convert("RGB")
            except OSError:
                print("damaged img: %s" % self.data["imgs"][idx])
                with open("data/kvqa/"+self.data["imgs"][idx],'rb') as f:
                    f=f.read()
                f=f+B'\xff'+B'\xd9'
                with open("data/kvqa/"+self.data["imgs"][idx],'wb') as f1:
                    f1.write(f)
                img=Image.open("data/kvqa/"+self.data["imgs"][idx]).convert("RGB")
            ...
            return q, ques_out, kg_out, self.transformer(img), ans
    ...
    def prepare_instance(self, datum, key, idx):
        ...
        self.imgs.append(datum["imgPath"])  
